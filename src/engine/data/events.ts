...
export const EVENTS: GameEvent[] = [
  ...
  {
    id: 'querosene', icon: 'fuel', title: 'Querosene dispara',
    text: 'A Petrobras anuncia alta de 25% no QAV a partir de segunda.',
    L: { label: 'Travar preço (hedge)', fx: { cash: -1 }, apply: (s) => {
  ...
  {
    id: 'guerra', icon: 'tag', title: 'Guerra tarifária',
    text: 'A concorrente anunciou passagens 30% mais baratas nas suas rotas.',
    L: { label: 'Acompanhar os preços', fx: { cash: -1 }, apply: (s) => {
    ...
    R: { label: 'Apostar no serviço', fx: { rep: 1, ops: -1 }, apply: (s) => {
  ...
  },
  {
    ...
    R: { label: 'Inspeção visual e seguir', fx: { fleet: -1 }, apply: (s) => {
      const p = firstAvailable(s);
      if (!p) return NOTHING;
      if (chance(s, 0.4)) {
  ...
  {
    id: 'sistema', icon: 'bug', title: 'Pane no sistema de reservas',
    text: 'O site caiu na sexta à noite. A TI estima 12 horas para reparar.',
    L: { label: 'Chamar consultoria externa', fx: { cash: -1 }, apply: (s) => {
      const c = 90000 * scaleCost(s);
      s.cash -= c;
      return `Sistema restaurado em 2 horas por ${fmtMoney(c)}.`;
    } },
    R: { label: 'Esperar a equipe interna', fx: { rep: -1, cash: -1 }, apply: (s) => {
  ...
  {
    id: 'slotbarato', icon: 'key', title: 'Slot à venda',
    ...
    R: { label: 'Comprar', fx: { cash: -1 }, apply: (s) => {
